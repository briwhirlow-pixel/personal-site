...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const invoiceUrl = `${siteUrl}/invoice/${id}?t=${signInvoiceId(id)}`;
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1A1A1A">
        <h2 style="margin-bottom:4px">Invoice ${escapeHtml(inv.invoice_number)}</h2>
        <p style="color:#737373;margin-top:0">Project: ${escapeHtml(inv.project_name)}</p>
        <hr style="border:none;border-top:1px solid #E5E4DF;margin:24px 0"/>
        <p>Hi ${escapeHtml(inv.client_name)},</p>
        <p>Please find your invoice for <strong>${escapeHtml(inv.project_name)}</strong> below.</p>
        ...
        ${inv.due_date ? `<p style="color:#737373;font-size:14px">Due by: <strong>${escapeHtml(new Date(inv.due_date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }))}</strong></p>` : ""}
        ${inv.payment_instructions ? `<div style="background:#F2F1EC;border-radius:8px;padding:16px;margin:24px 0"><p style="margin:0 0 6px 0;font-weight:700;font-size:13px">How to Pay</p><p style="margin:0;font-size:14px;color:#4A4A4A;white-space:pre-line">${escapeHtml(inv.payment_instructions)}</p></div>` : ""}
        ${inv.notes ? `<p style="font-size:14px;color:#737373">${escapeHtml(inv.notes)}</p>` : ""}
